...
gm1<-glm(y~nr.employed+euribor3m+emp.var.rate+pdays+previous+cons.price.idx+cons.conf.idx+campaign, family=binomial, data=dfw)
summary(gm1)
...
A través de la grafica Normal Q-Q, podem observa com el nou model s'ajusta molt més que l'anterior a una distribució normal i en podem identificar unes cues amb tendencia inferior respecte a la linia de la normal. 

Per altra banda, també podem observar com en "Residuals vs Leverage" la majoria del punts es concentra a la part esquerra de la grafica, ens indica que no tenim grans quantitas de valors influents. Tot i aixi podriem considerar les observacions 15581, 10038 i 3371 com a valor inclosos dins aquesta categoria.

TREURE INFLUENT DATA??

###Comparacio dels dos models
```{r}
...
Si el condes anterior ens ha donat com a variables factor significatives algunes que ja teniem en el model com a continues, hem de triar o una o altra versio. Si tenim dubtes entre agafar una variable com a continua o factoritzada, hem de fer el seg?ent:
